...
export default function ReferEarnPage() {
  ...
  const referralCode = "ASIRA-2026";

  useEffect(() => {
    const storedReferrals = Number(
      localStorage.getItem("asira_referrals") || "0"
    );

    const storedEarnings = Number(
      localStorage.getItem("asira_referral_earnings") || "0"
    );
    ...
    <main className="min-h-screen bg-[#f7f7f7] px-6 py-8 text-black">
      <div className="mx-auto max-w-sm">
        ...
        <div className="mt-6 rounded-3xl bg-yellow-50 p-5 text-sm text-yellow-800">
          Rewards are automatically credited to your Asira Wallet balance after successful referrals.
        </div>
      </div>
    </main>
  );

  ...
  async function handleSignup() {
    ...
    try {
      ...
      localStorage.setItem(
        "manny_pay_wallet_logged_in",
        "yes"
      );

      localStorage.setItem(
        "manny_pay_wallet_full_name",
        fullName
      );

      localStorage.setItem(
        "manny_pay_wallet_phone",
        mobile
      );

      localStorage.setItem(
        `manny_pay_wallet_balance_${mobile}`,
        "0"
      );
...
>
  ...
  className="mt-5 w-full text-sm text-[#1E3A8A] transition hover:text-[#2563EB]"
>
